...
// имя запроса используется для логирования и потенциально может
// использоваться еще где-то, например, для трейсинга
type Query struct {
	Name     string
	QueryRaw string
}

// NamedExecer - интерфейс для работы с именованными запросами
// с помощью тегов в структурах, например с помощью библиотеки scany
...
// TxManager менеджер транзакций, который выполняет
